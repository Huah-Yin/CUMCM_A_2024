function out = RunGA(problem, params)
    ...
    % 初始值矩阵
    initial_positions = zeros(100, 2);
    % 设置范围和步长
    r_d = 10000:10000:100000; % 第一列的值
    c_d = 0.1:0.1:1; % 第二列的值
    % 填充矩阵
    % 使用repmat和reshape来填充矩阵
    % initial_positions(:, 1) = repmat(r_d, 1, 10)'; % 重复第一列的值
    % initial_positions(:, 2) = repmat(c_d, 1, 10)'; % 重复第二列的值
    ...
    for i = 1:nPop
        ...
        % if i <= nPop / 2
        %     idx_i = mod(i, size(initial_positions, 1)) + 1;
        %     pop(i).position = initial_positions(idx_i, :);
        % else
        %%随机生成过程
