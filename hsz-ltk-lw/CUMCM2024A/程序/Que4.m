...
rec_v=zeros(223+1,3000);    % 每秒结果保存(m/s)
res_pot=zeros((223+1)*2,3000);   % 贴入excel的位置数据
while true
    t=t+1;
...
%         plot(xdata(1),ydata(1),"*r")
    ...
    for idx=1:223
        ...
        elseif state(idx)==2
            ...
            if temp1>temp2 && theta(idx)<boundary_theta+pi
                % 此节点未进入盘出曲线
                state(idx+1)=2;
                xout=-xin;
                yout=-yin;
    ...
    %% 绘图
    if t_in~=-1 && (t-t_in==0 || t-t_in==50 || t-t_in==100)
        figure
    ...
    if abs(mod(t*dt,1))<1e-10
        rec_x(:,round(t*dt)+1)=round(xdata(:,1)/100,6);
        rec_y(:,round(t*dt)+1)=round(ydata(:,1)/100,6);
        rec_v(:,round(t*dt)+1)=round(vdata(:,1),6);
        res_pot(1:2:end,round(t*dt)+1)=rec_x(:,round(t*dt)+1);
        res_pot(2:2:end,round(t*dt)+1)=rec_y(:,round(t*dt)+1);
    end
end
fprintf("结果存于变量res_pot、rec_v中")
fprintf("截取两变量最后101列写于excel中")


%% 画图
figure
t_ls=[t_in,t_in+50,t_in+100];
for i=1:3
    xdata=rec_x(:,t_ls(i))*100;
    ydata=rec_y(:,t_ls(i))*100;
    subplot(1,3,i)
    [x_std,y_std]=get_xy(get_curve(b,boundary_theta:0.1:20*pi),boundary_theta:0.1:20*pi);
    [x_circle,y_circle]=get_xy(return_radius,0:0.1:2*pi);
    S_x=[];
    S_y=[];
    for sss=0:0.1:calculate_return_length(return_radius)
    ...
    plot(xdata(:),ydata(:),"*",'Color',[0.9,0.1,0.1])
    axis square
    title(strcat(num2str(t_ls(i)-t_in),"s"))
end
