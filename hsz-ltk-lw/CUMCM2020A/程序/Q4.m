...
for k=1:G
    ...
    usetime=toc;
    fprintf("完成计算第%d代,适应度%f(面积%.3f,对称性指标%.2f),进度%.6f,用时%.2fs,预计还需%.3fmin\n",k,max_fit,Fit1(max_fit_idx),Fit2(max_fit_idx),k/G,usetime,usetime*(G-k)/60)
end
...
function [fit,fit1,fit2]=aim(x)
    ...
    while key(k_idx_umax+dk)>217 && key(k_idx_umax-dk)>217
        fit2=fit2+abs(key(k_idx_umax+dk)-key(k_idx_umax-dk));
        dk=dk+1;
    end
    fit2=fit2/dk;
...
function pc=get_pc(sta_num)
    % 交叉率，sta_num-稳定代数
    pc=min(0.7+sta_num*0.05,0.9);   % 防止pc超过0.9
end

function pm=get_pm(sta_num)
    % 变异率，sta_num-稳定代数
    pm=min(0.5+sta_num*0.05,0.9);   % 防止pm超过0.9
end
