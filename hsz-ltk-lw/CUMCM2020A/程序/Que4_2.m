...
%% 设置炉温
U_info4=[ones(1,5)*184.1790,196.1701,237.5578,ones(1,2)*264.6693,ones(1,2)*25];
...
y3_fig=[];
for x=x_fig
    y4_fig(end+1)=interp1(x0,y2,x);
    y3_fig(end+1)=interp1(x0,y3,x);
end
plot(x_fig,y3_fig,"k",x_fig,y4_fig,"r")
xlabel("炉内位置/cm")
ylabel("温度/℃")
xlim([0,435.5])
legend("问题3","问题4")
...
for idx=[2,1]
    ...
    k_step=0.005;     % 时间步长(s)
    v=[97.6667,90.7978];   % 传送带前进速度(cm/min)
