...
D_info=    1e-3*[0.1,0.5,ones(1,3)*0.5,  0.5,0.5,0.6,0.6,0.6,0.2,0.2];
sigma_info=[0.1, 2.2 ,ones(1,3)*3.3,    2.4,  3.5,  3.4,  2.5,2.5, 1.3,1.4];
len=30.5;
gap=5;
global x0 y0
x0=[0,25];
...
for k=2:k_max
    ...
    data(k,1)=-j_step*sigma*(data(k,2)-get_U(t_now*vv))+data(k,2);
    data(k,j_max)=-j_step*sigma*(data(k,j_max-1)-get_U(t_now*vv))+data(k,j_max-1);
end
%% 计算误差平方和
